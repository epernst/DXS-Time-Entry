page 62000 "DXS.Time Entry Setup"
{
    Caption = 'DXS Time Entry Setup';
    PageType = Card;
    SourceTable = "DXS.Time Entry Setup";
    DeleteAllowed = false;
    InsertAllowed = false;
    UsageCategory = Administration;
    AccessByPermission = tabledata "DXS.Time Entry Setup" = rmi;
    ...
    actions
    {
        area(Navigation)
        {
            action("DXS UpgradeNAV2017")
            {
                Caption = 'Upgrade from NAV 2017';
                ApplicationArea = All;
                Image = UpdateDescription;
                trigger OnAction();
                var
                    TimeMigration: Codeunit "DXS.Time Data Migration";
                begin
                    TimeMigration.MigrateFromNAV2017();
                end;
            }
            action("DXS HelpResources")
            {
                Caption = 'DXS Time Help Resources';
                Image = Picture;
                Promoted = false;
                ApplicationArea = Suite;
                RunObject = page "DXS.Time Help Resources";
